...
fetch_source() {
  ...
  if [[ "$(git -C "$dir" rev-parse HEAD 2>/dev/null || true)" != "$commit" ]]; then
    # fetch_source is called through command substitution below. Keep diagnostics on stderr so
    # stdout remains the single machine-readable source-directory value on a clean runner.
