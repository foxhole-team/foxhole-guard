...
"$GITLEAKS_BIN_PATH" git . \
  ...
  --no-banner \
  --report-format json \
  --report-path "${REPORT_PATH%.json}-history.json" || history_status=$?

if [[ "$working_tree_status" -ne 0 || "$history_status" -ne 0 ]]; then
  echo "gitleaks: working tree=$working_tree_status history=$history_status" >&2
  exit 1
fi
