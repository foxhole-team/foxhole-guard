...
install_target_app() {
  ...
  adb wait-for-device
  if [[ "$target_package" == "com.foxhole.guard.debug" ]]; then
  ...
  if ! adb shell pm path "$target_package" >/dev/null; then
    ...
    adb shell pm list packages 'com.foxhole.guard' >&2 || true
    exit 1
  fi
}

require_strict_release_target() {
  if [[ "$STRICT_RELEASE_GATE" != "1" ]]; then
    return
  fi
  if [[ "$TARGET_PACKAGE" == *.debug ]]; then
    echo "Strict release macrobenchmark requires a non-debug target package; got ${TARGET_PACKAGE}." >&2
    echo "Set FOXHOLE_ANDROID_TEST_TARGET_PACKAGE to the installed release package before enabling FOXHOLE_STRICT_RELEASE_MACROBENCHMARK=1." >&2
    exit 1
  fi
}
...
run_startup_benchmark() {
  ...
  run_macrobenchmark_with_log_gate startup \
    ...
    -Pandroid.testInstrumentationRunnerArguments.class=com.foxhole.guard.macrobenchmark.HomeMacrobenchmark#startup
  if [[ "$EVENT_NAME" == "pull_request" && "$STRICT_RELEASE_GATE" != "1" ]]; then
    echo "Skipping PR startup threshold gate on GitHub emulator; benchmark JSON and logcat gates remain uploaded as evidence."
  else
...
run_baseline_profile_generation() {
  ...
  "$GRADLEW" --no-daemon --console=plain --stacktrace :macrobenchmark:connectedCheck \
    ...
    -Pandroid.testInstrumentationRunnerArguments.class=com.foxhole.guard.macrobenchmark.FoxholeBaselineProfileGenerator
  baseline_source="$(
  ...
  if ! grep -q 'Lcom/foxhole/guard/' "$baseline_output_dir/baseline-prof.txt"; then
...
run_baseline_profile_generation_when_supported() {
  if [[ "$BASELINE_TARGET_PACKAGE" != "com.foxhole.guard.debug" ]]; then
...
if [[ "$EVENT_NAME" == "pull_request" ]]; then
  ...
  if [[ "$STRICT_RELEASE_GATE" != "1" ]]; then
    ...
    run_macrobenchmark_with_log_gate full-suite \
      "$GRADLEW" --no-daemon --console=plain --stacktrace :macrobenchmark:connectedCheck \
      -Pmacrobenchmark.targetPackage="$TARGET_PACKAGE" \
      -Pandroid.testInstrumentationRunnerArguments.class=com.foxhole.guard.macrobenchmark.HomeMacrobenchmark
  else
    ...
    run_macrobenchmark_with_log_gate full-suite \
      ...
      "-Pandroid.testInstrumentationRunnerArguments.foxhole.baselineProfileMode=require"
  fi
