...
FoxHole targets Android 14+ and declares `specialUse` foreground service type
for the VPN and local proxy services.
...
`Maintains a user-initiated VPN tunnel or local firewall guard so selected apps
and DNS traffic remain protected while Foxhole is not in the foreground.`
