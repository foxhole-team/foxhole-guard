...
class DnsFilterUpdateClientTest {
    ...
    fun `persists exact signed envelope only after all FoxCore checks pass`() =
        runBlocking {
            ...
            val phases = mutableListOf<RemoteUpdatePhase>()
            val progress = mutableListOf<RemoteDownloadProgress>()

            val result = fixture.client.update(
                MANIFEST_URL,
                store,
                onPhase = phases::add,
            ...
            assertEquals(fixture.artifactBytes.size.toLong(), progress.last().downloadedBytes)
            assertEquals(fixture.artifactBytes.size.toLong(), progress.last().totalBytes)
        }
