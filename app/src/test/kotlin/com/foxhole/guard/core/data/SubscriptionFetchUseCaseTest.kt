...
class SubscriptionFetchUseCaseTest {
    ...
    @Test
    fun `reads current underlying socket factory for every subscription fetch`() =
        runBlocking {
            var socketFactoryReads = 0
            val useCase =
                fetchUseCase(
                    client = scriptedClient("https://example.com/sub" to TestResponse.ok("payload")),
                    underlyingSocketFactory = {
                        socketFactoryReads += 1
                        null
                    },
                )
            ...
            assertEquals(2, socketFactoryReads)
        }

    @Test
    fun `binds subscription client to supplied underlying socket factory`() {
        val client = OkHttpClient()
        val socketFactory = RefusingSocketFactory()

        val bound = client.withUnderlyingSocketFactory(socketFactory)

        assertSame(socketFactory, bound.socketFactory)
        assertSame(client, client.withUnderlyingSocketFactory(null))
    }
    ...
    private fun fetchUseCase(
        client: OkHttpClient,
        underlyingSocketFactory: () -> SocketFactory? = { null },
    ...
    private class RefusingSocketFactory : SocketFactory() {
        override fun createSocket(): Socket = throw IOException("not used")

        override fun createSocket(
            host: String,
            port: Int,
        ): Socket = throw IOException("not used")
