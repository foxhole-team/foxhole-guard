...
class BoundedPublicHttpFetchTest {
    ...
    @Test
    fun `cancelling coroutine cancels active okhttp call`() =
        runBlocking {
            val started = CompletableDeferred<Unit>()
            val callWasCancelled = AtomicBoolean(false)
            val job =
                launch {
                    awaitCancellableHttpCall<Unit>(
                        cancelCall = { callWasCancelled.set(true) },
                        startCall = { started.complete(Unit) },
                    )
                }

            started.await()
            job.cancelAndJoin()
            assertTrue(callWasCancelled.get())
        }
