...
class AnomalyRetentionPolicyTest {
    ...
    @Test
    fun `network journal follows journal retention even when statistics use another window`() {
        val settings =
            Settings(
                expert = ExpertSettings(
                    networkActivityLogging = true,
                    diagnosticsRetentionPolicy = RetentionPolicy(RetentionPreset.CUSTOM, customDays = 3),
                ),
                statistics = StatisticsSettings(retention = StatisticsRetention.MONTH),
            )

        assertEquals(NOW_MS - 3L * DAY_MS, networkJournalRetentionCutoff(NOW_MS, settings))
    }
