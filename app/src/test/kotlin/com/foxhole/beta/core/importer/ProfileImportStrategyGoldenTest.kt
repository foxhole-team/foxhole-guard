...
class ProfileImportStrategyGoldenTest {
    ...
    @Test
    fun `raw sing box json strategy is selected after xray check`() {
        val result = parser.parseUserInputWithStrategy(rawSingBoxPayload())

        assertEquals(ProfileImportStrategyId.RAW_SING_BOX_JSON, result.strategyId)
        assertEquals(ProfileSourceType.RAW_SINGBOX_JSON, result.parsed.sourceType)
        assertEquals(ProtocolHint.SING_BOX, result.parsed.protocolHint)
    }

    @Test
    fun `json with protocol and type still uses xray before sing box`() {
        val result =
    ...
    @Test
    fun `malformed mixed payload remains rejected instead of dropping invalid lines`() {
        try {
            parser.parseUserInputWithStrategy(
                """
                ${vlessUri("valid")}
            ...
            fail("expected malformed mixed payload to be rejected")
        } catch (error: IllegalArgumentException) {
            assertTrue(error.message.orEmpty().contains("unsupported subscription entries"))
        }
    }
    ...
    private fun rawSingBoxPayload(): String =
        """
        {
          "outbounds": [
            {
