...
class LiveLocalFirewallGuardRuntimeTest {
    ...
    @Test
    fun enablingI2pBesideFirewallKeepsFirewallPolicyAndInternet() =
        runBlocking {
            val app = liveLocalGuardApp()
            val settings = app.container.settingsRepository
            val blockedPackage = firstInstalledPackageExcept(app.packageName)
            try {
                app.container.i2pTrafficRepository.clear()
                startLocalFirewallGuard(app, blockedPackage)
                ...
                assertTrue(
                    "enabling I2P replaced the firewall package policy",
                    app.container.runtimeConfigAssembler
                        .assembleLocalGuard(settings.current(), LocalGuardMode.FIREWALL)
                        .contains(blockedPackage),
                )
                assertTrue(
                    "DNS stopped resolving after I2P joined the firewall carrier",
                    withContext(Dispatchers.IO) {
    ...
    private suspend fun resetLocalGuardTestState(app: FoxholeApplication) {
        ...
        app.container.settingsRepository.updateI2pEngaged(false)
        app.container.settingsRepository.updateI2pEnabled(false)
    ...
    private suspend fun startLocalFirewallGuard(
        ...
        assertTrue(
            ...
            waitForCondition(timeoutMs = 25_000L) {
