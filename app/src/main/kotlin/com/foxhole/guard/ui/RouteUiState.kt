...
data class StatisticsRouteUiState(
    ...
    // Session-scoped I2P counter (an in-memory object, wiped on teardown), delivered by a late
    // combine so its ~1 Hz tick never re-runs the heavy dashboard build. I2P has NO persisted
    // attribution — no VpnMode.I2P, no I2P AppTunnelLane — so its row is honest only for the
    // CURRENT SESSION and must be labelled as such whatever window is picked. TOR needs no such
    // field: its bytes come from the per-app windows, which the summary slices by that window.
