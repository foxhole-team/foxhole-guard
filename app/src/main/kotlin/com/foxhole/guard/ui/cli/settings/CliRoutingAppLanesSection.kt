...
internal fun CliAssignedAppsSection(
    ...
    CliPanel(
        ...
        if (assigned.isEmpty()) {
            Text(
                text = stringResource(R.string.cli_route_apps_empty),
                style = CliType.small,
                color = colors.dim,
            )
        }
...
private fun CliAssignedAppRow(
    ...
    CliDropdownRow(
        ...
        label = label,
        value = appLaneLabel(lane),
        options = APP_LANE_OPTIONS.map { candidate ->
            CliDropdownOption(
                id = candidate.name,
                label = appLaneLabel(candidate),
...
private fun appLaneLabel(lane: AppTunnelLane): String = when (lane) {
    AppTunnelLane.TOR -> "tor"
    AppTunnelLane.VPN -> "vpn"
    AppTunnelLane.BLOCK -> "block"
    AppTunnelLane.EXCLUDE -> "exclude"
}
