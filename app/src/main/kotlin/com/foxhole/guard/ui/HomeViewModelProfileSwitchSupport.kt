...
/**
 * Home quick-selector tap on a SINGLE (non-smart) profile. While a tunnel is live any pick reconnects
 * onto the chosen profile — confirmed first; while idle the
 * profile is activated and connected straight away. (Smart profiles do not reach here: the selector
 * expands their inline protocol picker instead.)
 */
internal fun HomeViewModel.onQuickSelectorSingleProfileSelected(profileId: Long) {
    ...
    if (state.connection.isPrimaryConnectionRuntime()) {
        promptSwitchProfileWhileConnected(profileId)
    } else {
        activateAndConnectProfile(profileId)
    }
}
...
// The plain "turn this profile on" path: make it active and connect through the same permission /
// proxy handling the split Start button uses (connect() itself sets the active profile).
