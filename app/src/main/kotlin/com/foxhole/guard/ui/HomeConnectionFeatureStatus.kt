...
/**
 * The LAN proxy pill, from the core's own listener state.
 *
 * This used to render `allowLanAccess` — the saved switch — which meant a proxy that never bound
 * (no Wi-Fi, empty password, a WireGuard profile with no stream outbound to relay into, a core that
 * refused the network) still showed green while nothing on the network could reach it. ON is now
 * exactly "the listeners are bound", and every asked-for-but-not-serving case is the amber PENDING
 * that the firewall and I2P pills already use for the same situation. The switch being off is the
 * only OFF.
 */
