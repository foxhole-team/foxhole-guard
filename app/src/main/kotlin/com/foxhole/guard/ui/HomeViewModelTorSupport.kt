...
/**
 * MODE-button entry point. On a live VPN tunnel a Tor-involving mode change is confirmed through
 * a countdown modal; when idle, tor-only, or Tor-neutral it applies straight away.
...
internal fun HomeViewModel.onConnectModeSwitchRequested(
    ...
    val snapshot = container.connectionController.snapshot.value
    if (!snapshot.isPrimaryConnectionRuntime()) {
        // Idle or tor-only: no live-VPN modal. The tor-only ⇆ vpn transitions keep their own
        // prompts; a plain preset selection just persists the mode for the next start.
...
internal fun HomeViewModel.confirmLiveModeSwitch(prompt: TorTransitionPrompt.LiveModeSwitch) {
    ...
    when (prompt.kind) {
        ...
        -> onRoutingModePresetSelected(prompt.target, prompt.scope)
        ...
        LiveModeSwitchKind.TOR_STOPS_VPN ->
            viewModelScope.launch {
                ...
                startRoutingMode(prompt.target, prompt.scope)
            }
    }
}
