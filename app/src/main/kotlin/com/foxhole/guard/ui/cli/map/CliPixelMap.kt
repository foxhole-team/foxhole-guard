...
internal fun CliPixelMap(
    ...
    val landBitmap by produceState<ImageBitmap?>(null, assetState, colors) {
        ...
        value = CliMapLandCache.bitmap(
            ...
            // Land is the logo's blue-graphite dark: panel with a hint of portal cyan, borders a
            // touch brighter. Clearly lighter than the panel background without fighting the
            // neon routes.
            color = lerp(colors.panel, colors.vpn, 0.10f),
            boundaryColor = lerp(colors.panel, colors.vpn, 0.22f),
            pointStride = 2,
        )
    }
    ...
    Canvas(
        ...
        destinations.forEach { destination ->
            ...
            val color = if (blinkOn) colors.accent else colors.accent.copy(alpha = 0.35f)
        ...
        if (originAvailable) {
            ...
            drawMarkerHalo(cell, colors.accent, scale, offsetX, offsetY)
...
/** Orange halo around the "you" cross: four diagonal cells. */
