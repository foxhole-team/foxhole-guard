...
internal class RuntimeSettingUpdateBarrier {
    ...
    fun reserve(): RuntimeSettingUpdateTicket {
        val completion = CompletableDeferred<Unit>()
        val previous =
            synchronized(lock) {
                tail.also { tail = completion }
            }
        return RuntimeSettingUpdateTicket(previous, completion)
    }
...
internal class RuntimeSettingUpdateTicket(
    private val previous: Deferred<Unit>,
    private val completion: CompletableDeferred<Unit>,
) {
    suspend fun awaitTurn() = previous.await()
