...
import com.foxhole.core.model.ProfileComparisonUiItem
...
import com.foxhole.core.model.TransportStatisticsUiItem
...
internal fun protocolStatistics(
    ...
    state.profiles.flatMap(Profile::statisticsProtocolHints).forEach { protocol ->
        if (protocol.isDisplayableVpnProtocol()) {
    ...
    profileTraffic.forEach { traffic ->
        if (traffic.protocolHint.isDisplayableVpnProtocol()) {
    ...
    return accumulators
        ...
        .filter { item -> item.protocol.isDisplayableVpnProtocol() && (item.successCount > 0 || item.totalBytes > 0L) }
        .sortedWith(
...
internal fun appTrafficRows(
    ...
    return com.foxhole.guard.statistics.appTrafficRows(
...
internal fun dnsProtectionSummary(
    ...
    return com.foxhole.guard.statistics.dnsProtectionSummary(
...
internal fun enabledDnsProtectionCategories(settings: DnsSettings): List<DnsProtectionCategory> =
    com.foxhole.guard.statistics.enabledDnsProtectionCategories(settings)

internal fun splitDnsBlockedByCategory(
    blocked: Int,
    categories: List<DnsProtectionCategory>,
): List<DnsProtectionCategoryRow> {
    return com.foxhole.guard.statistics.splitDnsBlockedByCategory(blocked, categories)
}
...
internal fun countryTrafficRows(
    ...
    com.foxhole.guard.statistics.countryTrafficRows(
...
internal fun normalizedCountryCode(countryCode: String?): String? =
    com.foxhole.guard.statistics.normalizedCountryCode(countryCode)

internal fun countryDisplayName(countryCode: String): String =
    com.foxhole.guard.statistics.countryDisplayName(countryCode)
