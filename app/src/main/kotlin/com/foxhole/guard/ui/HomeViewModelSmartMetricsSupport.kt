...
private fun HomeViewModel.startSmartProfileMetricsRefresh(
    ...
    protocolMetrics.markRefreshStarted(profileId)
    val refreshJob =
        viewModelScope.launch(start = kotlinx.coroutines.CoroutineStart.LAZY) {
            ...
            try {
                ...
                if (initialRestoreProfileId != null) {
                    restoreSubscriptionPrepared = initialSnapshot.profileId == profileId
                    restoreProfileId =
                        if (restoreSubscriptionPrepared) preparedProfileId else initialRestoreProfileId
                    restoreOptionId =
                ...
                protocolMetricsRefreshingOptionIdByProfileIdMutable.value =
                    protocolMetricsRefreshingOptionIdByProfileIdMutable.value +
                    (preparedProfileId to candidates.first().optionId)
                protocolMetricsRefreshingProfileIdsMutable.value =
                    protocolMetricsRefreshingProfileIdsMutable.value + preparedProfileId
                    ...
                    if (protocolMetricsRestoreOnCancel) {
                        ...
                        emitSuccess(
                            getApplication<Application>().getString(
                                R.string.protocol_metrics_refresh_cancelled,
                            ),
                        )
                    }
                }
                ...
                container.diagnosticsLogger.record(
                    "auto-connect",
                    "protocol refresh failed: ${diagnosticFailureLabel(error)}",
                )
                ...
                withContext(NonCancellable) {
                    ...
                    protocolMetricsRefreshingProfileIdsMutable.value =
                        protocolMetricsRefreshingProfileIdsMutable.value - setOf(profileId, preparedProfileId)
                    protocolMetricsRefreshingOptionIdByProfileIdMutable.value =
                        protocolMetricsRefreshingOptionIdByProfileIdMutable.value - preparedProfileId
                    if (protocolMetricsRefreshJob == coroutineContext[kotlinx.coroutines.Job]) {
                        protocolMetricsRefreshJob = null
                    }
                    protocolMetricsRestoreOnCancel = true
    ...
    protocolMetricsRefreshJob = refreshJob
    refreshJob.start()
}
...
private suspend fun HomeViewModel.runSmartProfileMetricsProbes(
    ...
    var previousVpnNetworkHandle = container.connectionController.currentVpnNetworkHandle()
...
internal fun protocolTestRestoreProfileId(snapshot: ConnectionSnapshot): Long? =
    snapshot.profileId?.takeIf { profileId ->
        snapshot.state in ACTIVE_CONNECTION_STATES &&
            profileId != FoxholeVpnService.LOCAL_GUARD_PROFILE_ID
    }

internal fun shouldRestoreAfterProtocolTest(
