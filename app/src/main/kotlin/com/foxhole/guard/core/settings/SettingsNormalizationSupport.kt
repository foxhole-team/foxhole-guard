...
internal fun Settings.normalized(): Settings {
    ...
    return copy(
        ...
        privacyRoute =
        privacyRoute
            .normalized(torLaneEmpty = expert.packages(AppTunnelLane.TOR).isEmpty())
...
private fun ExpertSettings.normalized(
    ...
    val normalizedPendingQuarantinePackages =
        pendingQuarantinePackages
            .filter { packageName -> packageName in normalizedBlockedPackages }
            .distinct()
    val normalized =
        copy(
...
private fun PrivacyRouteSettings.normalized(torLaneEmpty: Boolean): PrivacyRouteSettings =
    copy(
        // Selected-apps with nothing picked cannot run (it dead-ended the Tor window on "select
        // apps first") — coerce back to whole-device; picking apps re-arms the scope.
        scope =
        if (scope == PrivacyRouteScope.SELECTED_APPS && torLaneEmpty) {
