...
interface AnomalyDao {
    ...
    @Query(
        ...
        where notificationShown = 0 and createdAtMs >= :since and type != :excludedType
        order by createdAtMs asc, id asc
        limit :limit
        """,
    )
    suspend fun undeliveredAnomalyEvents(
        since: Long,
        limit: Int,
        excludedType: String,
    ): List<AnomalyEventEntity>

    @Query("update anomaly_events set notificationShown = 1 where id = :id and notificationShown = 0")
    suspend fun markAnomalyNotificationShown(id: Long): Int
    ...
    @Query(
        "select * from anomaly_events where createdAtMs >= :cutoff and type != :excludedType " +
            "order by createdAtMs desc, id desc",
    )
    fun observeAnomalyEvents(
        cutoff: Long,
        excludedType: String,
    ): Flow<List<AnomalyEventEntity>>
...
interface I2pTrafficDao {
    @Query(
        "insert or ignore into i2p_traffic_buckets (hourStartMs, ownBytes, transitBytes) values (:hourStartMs, 0, 0)"
    )
    suspend fun ensureBucket(hourStartMs: Long)

    @Query(
        """
        update i2p_traffic_buckets
        set ownBytes = ownBytes + :ownDelta,
            transitBytes = transitBytes + :transitDelta
        where hourStartMs = :hourStartMs
        """,
    )
    suspend fun addToBucket(
        hourStartMs: Long,
        ownDelta: Long,
        transitDelta: Long,
    )

    @Query("insert or ignore into i2p_traffic_totals (id, ownBytes, transitBytes) values (:id, 0, 0)")
    suspend fun ensureTotals(id: Int)

    @Query(
        """
        update i2p_traffic_totals
        set ownBytes = ownBytes + :ownDelta,
            transitBytes = transitBytes + :transitDelta
        where id = :id
        """,
    )
    suspend fun addToTotals(
        id: Int,
        ownDelta: Long,
        transitDelta: Long,
    )

    @Query("select * from i2p_traffic_buckets where hourStartMs >= :cutoff order by hourStartMs desc")
    fun observeBucketsSince(cutoff: Long): Flow<List<I2pTrafficBucketEntity>>

    @Query("select * from i2p_traffic_totals where id = :id limit 1")
    fun observeTotals(id: Int): Flow<I2pTrafficTotalEntity?>
    ...
    @Query(
        ...
        delete from i2p_traffic_buckets where hourStartMs not in (
            select hourStartMs from i2p_traffic_buckets order by hourStartMs desc limit :keep
        )
        """,
    )
    suspend fun trimBucketsTo(keep: Int)

    @Query("delete from i2p_traffic_buckets")
    suspend fun deleteAllBuckets()

    @Query("delete from i2p_traffic_totals")
...
interface WebAppDao {
    ...
    @Query("update web_apps set route = :route where id = :id")
    suspend fun updateRoute(id: Long, route: String)
