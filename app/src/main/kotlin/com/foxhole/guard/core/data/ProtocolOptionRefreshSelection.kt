...
internal fun ParsedSubscriptionProfile.resolveRefreshSelectedProtocolOptionId(
    ...
    if (previousSelectedId != null) {
        ...
        return selectedProtocolOptionId
            .normalizedProtocolOptionId()
            ?.takeIf { selectedId -> protocolOptions.any { option -> option.id == selectedId } }
            ?: protocolOptions.firstOrNull { option -> option.enabled }?.id
            ?: protocolOptions.firstOrNull()?.id
    }
