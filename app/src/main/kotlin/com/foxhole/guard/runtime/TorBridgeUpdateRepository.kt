...
class TorBridgeUpdateRepository(
    ...
    suspend fun refreshNow(onPhase: (RemoteUpdatePhase) -> Unit = {}): TorBridgeUpdateResult {
        val useFoxholeSource = settingsRepository.current().privacyRoute.bridgesUseFoxholeSource
        val result = client.update(store, useFoxholeSource, onPhase)
        when (result.status) {
            ...
            TorBridgeUpdateStatus.FAILED -> {
                ...
                diagnosticsLogger.record("tor", "bridge list update failed: ${result.reason.orEmpty()}")
            }
        }
        return result
    }
