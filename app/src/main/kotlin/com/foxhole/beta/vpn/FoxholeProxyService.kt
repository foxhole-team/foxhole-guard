...
class FoxholeProxyService : Service(), RuntimeServiceHost {
    ...
    override fun onStartCommand(
        ...
        handleForegroundRuntimeCommand(
            ...
            disconnectWithOptions = { commandStartId, _, preserveSmartStartAnalysis ->
                disconnect(commandStartId = commandStartId, preserveSmartStartAnalysis = preserveSmartStartAnalysis)
            },
            reload = ::reload,
    ...
    override fun onDestroy() {
        ...
        val hadActiveRuntime = activeSession != null || snapshot.isActiveRuntimeFor(TrafficMode.PROXY)
        super.onDestroy()
        stopTrafficUpdates()
        stopGeoRefresh()
    ...
    private suspend fun failClosedTeardown(
        ...
        val snapshot = FoxholeVpnRuntimeBridge.snapshot.value
        val session = activeSession
        val hadActiveRuntime = session != null || snapshot.isActiveRuntimeFor(TrafficMode.PROXY)
        if (!hadActiveRuntime && snapshot.isActiveRuntimeForAnotherMode(TrafficMode.PROXY)) {
            container.diagnosticsLogger.record(
                "connection",
    ...
    private suspend fun acceptProxyEgressFallback(
        session: VpnSession,
        reason: String,
        failure: Throwable?,
    ): Boolean {
        ...
        val egressProbe =
            runCatching {
                probeConnectivityEndpointsOverLocalProxy(
                    proxy = proxyAccess,
                    callTimeoutMs = PROXY_EGRESS_FALLBACK_CALL_TIMEOUT_MS,
                )
            }
        if (egressProbe.isFailure) {
            container.diagnosticsLogger.record(
                "dns",
                "proxy egress fallback rejected: endpoint probe failed reason=$reason error=${egressProbe.exceptionOrNull()?.message.orEmpty()}",
            )
            return false
        }
        ...
        container.diagnosticsLogger.recordStructured(
            "dns",
            "proxy egress fallback accepted",
            "reason=$reason",
    ...
    private companion object {
        ...
        private const val PROXY_EGRESS_FALLBACK_CALL_TIMEOUT_MS = 1_500L
