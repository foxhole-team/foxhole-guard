...
val foxCoreSourceRoot =
    providers.gradleProperty("foxhole.foxCoreSourceRoot")
        ...
        ?.takeIf(String::isNotEmpty)
        ?.let(rootProject::file)
        ?: rootProject.projectDir.parentFile.resolve("foxhole-guard-rust-core")
...
// libi2pd.so is built from the vendored i2pd source rather than committed. Without this a clean
...
val prepareFoxCoreNative = tasks.register("prepareFoxCoreNative") {
    group = "build"
    description = "Build the pinned FoxCore Rust JNI library for the two shipped phone ARM ABIs."
...
val publicReleasePreflight = tasks.register("publicReleasePreflight") {
    ...
    doLast {
        ...
        require(lastUploadedVersionCodeProvided) {
            "publicReleasePreflight needs -Pfoxhole.lastUploadedVersionCode=<code currently on Play> so the " +
                "monotonicity check verifies against the real uploaded code instead of the hardcoded default. " +
                "Wire it into release.yml from the committed/last-published value."
        }
...
val verifyReleaseJniSurface = tasks.register("verifyReleaseJniSurface") {
    ...
    doLast {
        val jniMembers = listOf("protectSocket")
        val apks =
...
android {
    ...
    defaultConfig {
        ...
        targetSdk = 37
        versionCode = 68
        // Public release line. The code keeps climbing from the developer-preview line (which ended
        // at 63 / "0.0.1-dev.60-opt.1-front.1") so publicReleasePreflight's monotonicity check holds
        // against anything ever uploaded; the name must never carry "-dev"/"debug"/"internal".
        // 64 / 1.1.0-beta1: web apps module (frame, watchdog, widgets), unified extras, vault.
        // 65 / 1.1.0-beta2: public UI and localization polish.
        // 67 / 1.1.0-beta4: arm64-v8a only for the first public beta, VLESS interop fix.
        // 66 / 1.1.0-beta3: FoxCore-only Rust runtime and direct UI integration.
        // 68 / 1.1.0-beta5: public-beta text and metadata pass, public CI trim.
