...
internal class MutableRouteTranslation(
    ...
    private val routedTransports = mutableMapOf<String, MutableSet<String>>()
    ...
    fun recordPackageRoute(
        packageName: String?,
        transport: String?,
    ) {
        if (packageName == null || transport == null) return
        routedTransports.getOrPut(packageName, ::linkedSetOf).add(transport)
    }

    fun hasCompleteTransportRoute(packageName: String): Boolean =
        routedTransports[packageName]?.containsAll(setOf("tcp", "udp")) == true
}
