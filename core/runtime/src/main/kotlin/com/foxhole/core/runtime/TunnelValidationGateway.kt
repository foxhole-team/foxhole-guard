...
class TunnelValidationGateway(
    ...
    private val currentTorProbeProxy: () -> TorProbeProxyLease? = { null },
    private val currentTorProbeIssue: () -> TorProbeProxyIssue? = { null },
) {
    ...
    suspend fun refreshDeviceIpInfo(fetchMode: IpInfoFetchMode): IpInfo {
        ...
        return fetchDeviceIpInfo(
            ...
            fetchMode = fetchMode,
            requestNetwork = upstreamNetwork,
            requireRequestNetwork = true,
            proxy = null,
        ).withDnsServers(
            localDnsServers = connectivityManager.dnsServerAddresses(upstreamNetwork),
    ...
    suspend fun refreshTorRouteIpInfo(fetchMode: IpInfoFetchMode): IpInfo {
        ...
        val info = ipInfoRepository.fetchVerifiedTorExit(
            callTimeoutMs = activeTunnelRuntimeProxyIpRefreshCallTimeoutMs(fetchMode),
            proxy = lease.access,
            resolverNetwork = currentUpstreamNetwork(),
        )
        requireCurrentTorProbeLease(started = lease, current = currentTorProbeProxy())
        return info
    }
        ...
        when {
            ...
            requestNetwork != null ->
                fetchDeviceIpInfoFromNetwork(
                    ...
                    fetchMode = fetchMode,
                    requestNetwork = requestNetwork,
                    allowDefaultFallback = !requireRequestNetwork,
                )
    ...
    private suspend fun fetchDeviceIpInfoFromNetwork(
        ...
        runCatching {
            ...
            if (!allowDefaultFallback || !shouldFallbackAppOwnedNetworkRequest(error)) {
                throw error
            }
