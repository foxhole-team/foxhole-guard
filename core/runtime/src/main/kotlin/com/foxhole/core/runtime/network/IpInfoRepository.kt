...
class IpInfoRepository(
    ...
    /**
     * Sockets pooled on the previous default network keep serving keep-alive requests after a
     * WiFi<->cell switch, so a whoami fetch can describe the OLD network's egress for minutes.
     * Derived clients share the base client's connection pool — one evict covers them all; the
     * per-network client cache is dropped too so stale [Network] handles never resolve DNS again.
     * Call off the main thread: evicting closes sockets.
     */
    ...
    private suspend fun fetchSingle(
        ...
        return execute(
            ...
            require(response.isSuccessful) { "ip info request failed: ${response.code}" }
            parseIpInfoResponse(
                body = response.body?.readIpInfoBodyCapped().orEmpty(),
                json = json,
                requireTorExitProof = requireTorExitProof,
            ).withLocalGeoCountry()
        }
