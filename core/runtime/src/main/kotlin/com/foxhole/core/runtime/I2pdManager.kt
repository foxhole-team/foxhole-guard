...
interface I2pdManager {
    ...
    /** Waits until the local SOCKS proxy accepts the per-start credentials. */
...
class I2pdProcessManager internal constructor(
    ...
    private suspend fun ensureStartedLocked(settings: I2pSettings): I2pdEndpoints {
        ...
        val endpoints = allocation.endpoints
        ...
        val ready =
            withTimeoutOrNull(timeoutMs.coerceAtLeast(1L)) {
                while (true) {
                    ...
                    if (accepts && publishReadyPhase(session)) {
    ...
    private fun publishReadyPhase(session: I2pdReadySession): Boolean =
        synchronized(stateLock) {
