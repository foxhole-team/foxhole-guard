...
#define path_to_file(file) "~/.i2pd/" #file " or /var/lib/i2pd/" #file
	...
	void Init()
	{
		...
		socksproxy.add_options()
			...
			("socksproxy.username", value<std::string>()->default_value(""),           "SOCKS5 username; enables required user/password authentication")
			("socksproxy.password", value<std::string>()->default_value(""),           "SOCKS5 password; enables required user/password authentication")
