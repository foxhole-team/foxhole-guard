...
namespace client
{
	...
	void ClientContext::ReadSocksProxy ()
	{
		...
		if (socksproxy)
		{
			...
			std::string socksProxyUsername;    i2p::config::GetOption("socksproxy.username",         socksProxyUsername);
			std::string socksProxyPassword;    i2p::config::GetOption("socksproxy.password",         socksProxyPassword);
